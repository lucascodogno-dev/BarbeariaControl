...
const Dashboard = () => {
  ...
  useEffect(() => {
    ...
    socket.on("slot-blocked", fetchAppointments);
    socket.on("schedule-updated", fetchAppointments);
    socket.on("slot-unblocked", fetchAppointments);
  ...
  useEffect(() => {
    ...
    appointments.forEach((appointment) => {
      ...
      if (appointment.status !== "cancelado") {
        const revenue =
          ...
          serviceMap[appointment.serviceId]?.price ||
          0;
    ...
    const serviceDistributionData = Object.keys(serviceCountMap).map(
      (serviceId) => {
                    ...
                    <div className="flex justify-between items-start">
                      <div>
                        ...
                        <div className="text-sm text-gray-500 dark:text-gray-400">
                          {appointment.serviceName || "Serviço desconhecido"} -{" "}
                        ...
                        <div className="flex items-center mt-2 text-sm text-gray-600 dark:text-gray-300">
                          ...
                          />
                          <span>
                            {formatDateToBR(appointment.date)} às{" "}
                            {appointment.time}
                          </span>
                        </div>
                      </div>
